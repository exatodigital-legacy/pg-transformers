...
run time the kernel quantizes each linear's input activations per token and
per column block, and computes the GEMM in the integer domain. The baseline
SIMD build uses exact u8 activations (128-column blocks) and
`i32x4.dot_i16x8_s`; the relaxed-SIMD build quantizes to u7 (64-column
blocks), which is the relaxed int8 dot's deterministic envelope, so each
16 columns cost a single SDOT on Arm. On x86 `pgt_load` keeps int8 models
on the baseline kernel, which V8 lowers better there. This makes the int8
variants the fastest ones. The port is no longer bit-exact, so `verify`
